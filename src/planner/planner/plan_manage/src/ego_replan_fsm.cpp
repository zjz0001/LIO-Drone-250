...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    target_check_timer_ =  nh.createTimer(ros::Duration(0.05), &EGOReplanFSM::checkTargetExistCallback, this);
    ...
    have_target_pub_ = nh.advertise<std_msgs::Bool>("planning/target_exist", 100);
  ...
  void EGOReplanFSM::checkTargetExistCallback(const ros::TimerEvent &e)
  {
    if(have_target_ == 0){
      std_msgs::Bool flag_target_exist;
      flag_target_exist.data = false;
      have_target_pub_.publish(flag_target_exist);
    }
  }
