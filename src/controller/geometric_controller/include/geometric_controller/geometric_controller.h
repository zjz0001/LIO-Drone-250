...
private:
  ...
  ros::Subscriber ttargetposeSub_, targetexistSub_;
  ...
  void
  multiDOFJointCallback(const trajectory_msgs::MultiDOFJointTrajectory &msg);
  void keyboardCallback(const geometry_msgs::Twist &msg);
  void cmdloopCallback(const ros::TimerEvent &event);
  void mavstateCallback(const mavros_msgs::State::ConstPtr &msg);
    
  void ttargetposeCallback(const geometry_msgs::PoseStamped &msg);
  void targetexistCallback(const std_msgs::Bool &msg);
  ...
  vector3d2PoseStampedMsg(Eigen::Vector3d &position,
  ...
  Eigen::Vector4d geometric_attcontroller(const Eigen::Vector4d &ref_att,
                                          const Eigen::Vector3d &ref_acc,
                                          Eigen::Vector4d &curr_att);

  bool cla_distance(const Eigen::Vector3d &target_pos);
